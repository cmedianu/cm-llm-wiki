...
**Mass renames:** if many wikilinks broke at once because a folder or naming convention changed (e.g. the user kebab-cased everything, or moved a folder), don't fix them one at a time. Use `../wiki-scripts/kebab-rename.py` — it does kebab-case file rename + wikilink rewrite + manifest re-key in lockstep, has a `LEGACY_WIKILINK_TARGETS` knob for old paths that should still resolve, and a `DRY_RUN=1` preview. Suitable for any mass-rename, not just kebab-case — adapt the slugify or the `PATH_OVERRIDES` to match the rename you need.
