...
If the manifest gets out of sync with the filesystem — typically after a bulk operation done outside the skills (Obsidian-side rename, manual `mv`, restore from backup) — don't rebuild the whole wiki. Run `../wiki-scripts/regen-manifest.py` from the vault root instead. It walks the filesystem and rewrites the manifest while preserving `ingested_at` for sources that already had entries. `DRY_RUN=1` previews. This is the cheap repair; full rebuild is the expensive one.
